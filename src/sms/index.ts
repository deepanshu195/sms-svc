...
import { Logger } from '../utils/basicInterface';
class SmsManager {
    vendors: Array<SmsBase>;
    logger: Logger;
    constructor() {
        this.logger = console;
        this.vendors = [];
    }
    async sendTransactionalMessage(message: string, phone: number, priorityArray?: Array<string>): Promise<any> {
    ...
    async sendOtpMessage(message: string, phone: number, priorityArray?: Array<string>): Promise<any> {
        helper.checkForInputValue(message, phone);
        const vendorsList = this.sendPriorityWise(priorityArray);
        this.logger.log(
            helper.loggerSuffix +
    ...
    async sendPromotionalMessage(message: string, phone: number, priorityArray?: Array<string>): Promise<any> {
        helper.checkForInputValue(message, phone);
        const vendorsList = this.sendPriorityWise(priorityArray);
        this.logger.log(
            helper.loggerSuffix +
    ...
    addVendors(config: any, logger: Logger): void {
        this.logger = logger;
    ...
    sendPriorityWise(priorityArray?: Array<string>): any[] {
        let fillArrayPriorityWise: Array<SmsBase>;
        if (priorityArray) {
